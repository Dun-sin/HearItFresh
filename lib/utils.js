...
export const getAllTracks = async (albums, numTracks) => {
  ...
  const subTracks = tracks.reduce((acc, item) => {
  ...
  const removeEmptyObjects = subTracks.map(subArr => subArr.filter(obj => Object.keys(obj).length !== 0));
  ...
  const allTracksID = result.map(item => item.uri).filter(item => item != undefined)
